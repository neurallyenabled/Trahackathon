...
html = open("/home/mj/projects/Trahackathon/ExampleHTMLs/aboutus.html","r", encoding="UTF-8")
...
#        htmlfile.extend(tag.string.split())
#print(htmlfile)
#for word in htmlfile:
#    if validate_email(word) == True:
#        print(f"{str(word)} is an email")
#    elif validate_domain(word) == True:
#        print(f"{str(word)} is a domain")
        ...
        if word == "|": 
           print(word)
            ...
            try: 
                ...
                tag.find(text=re.compile(str(word))).replace_with(new_link)
                link = tag.find('a', href=word)
                link.string = word
            except idna.IDNAError as e:   
